...
- 264 static English/Turkish pages: full catalog, equipment groups, 54 product/software details per language, application/industry directories, 18 fixture collections, the document library, and the existing Labotens family pages
...
- Twelve bilingual application guides with detailed method variants, practical workflows, calculation examples, reporting guidance, FAQs, two explanatory diagrams per guide, curated equipment and fixture links, and primary technical references
...
Resources links to the document library. Company and Software & Services now have dedicated bilingual directories and pages for every menu item; Falcon overview retains its existing product page. Content and source decisions are in `data/company-services.mjs` and `data/company-services-review.md`, rendered by `tools/company-service-pages.mjs`. The supplied logo file is unchanged. The shared palette uses its golden yellow, graphite and silver tones, with darker gold for readable text on light backgrounds. See `IMPLEMENTATION-PLAN.md` for the implemented scope and remaining work.
...
1. Expand detailed specifications and fixture content, review company/service content and expand remaining resource pages, and approve technical content and translations.
